...
export const useBusinessData = () => {
  ...
  const addTransaction = async (tx: Transaction) => {
    if (useFirebase) {
      const cleanTx: Transaction = { ...tx };

      Object.keys(cleanTx).forEach((key) => {
        if (cleanTx[key as keyof Transaction] === undefined) {
          delete cleanTx[key as keyof Transaction];
        }
      });

      await firebaseService.addTransaction(cleanTx);
